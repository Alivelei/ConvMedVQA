...
"""
    @Time : 2023/6/21 11:52 
    @Author : smile 笑
    @File : efficient_v1.py
    @desc : framework.model.efficient_mix.
"""
...
from framework.model.efficient_mix.network.word_embedding import WordEmbedding
from framework.model.efficient_mix.network.hidden_mix import BalancedTransMix, SoftTransMix, HardTransMix
...
class EfficientNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 bn_momentum=0.01,
                 pretrained=False,
                 progress=False, qus_embedding_dim=300,
        ...
        self.qus_emb = QusEmbeddingMap(glove_path, word_size, qus_embedding_dim, init_conv_out_channels)

        self.img_attn = MultiAttnFusion(init_conv_out_channels)
        ...
        self.qus_linear = nn.Sequential(nn.Linear(init_conv_out_channels, head_conv_out_channels),
                                        nn.LayerNorm(head_conv_out_channels),
                                        nn.GELU())

        self.avpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(head_conv_out_channels, ans_size)
    ...
    def get_features(self, x, y, label):
        self.check_input(x)
        y = self.qus_emb(y)
        ...
        if self.training:
            return out, y, mix_label
        else:
            return out, y

    def forward(self, x, y, label=None):
        if self.training:
            x_out, y, mix_label = self.get_features(x, y, label)
        else:
            x_out, y = self.get_features(x, y, label)
...
# 4293211 -> 4450567
def mix_hid_efficient_b0(**kwargs):
    return EfficientNet(0, qus_embedding_dim=300, ans_size=kwargs["ans_size"], glove_path=kwargs["glove_path"],
...
# 6798847 -> 6956203
def mix_hid_efficient_b1(**kwargs):
    return EfficientNet(1, qus_embedding_dim=300, ans_size=kwargs["ans_size"], glove_path=kwargs["glove_path"],
                        word_size=kwargs["word_size"], select_mix_flag=kwargs["select_mix_flag"],
                        mix_probability=kwargs["mix_probability"], mix_alpha_1=kwargs["mix_alpha_1"],
                        mix_alpha_2=kwargs["mix_alpha_2"])
...
# 8015201 -> 8177037
def mix_hid_efficient_b2(**kwargs):
    return EfficientNet(2, qus_embedding_dim=300, ans_size=kwargs["ans_size"], glove_path=kwargs["glove_path"],
                        word_size=kwargs["word_size"], select_mix_flag=kwargs["select_mix_flag"],
                        mix_probability=kwargs["mix_probability"], mix_alpha_1=kwargs["mix_alpha_1"],
                        mix_alpha_2=kwargs["mix_alpha_2"])
...
# 11038983 -> 11226371
def mix_hid_efficient_b3(**kwargs):
    return EfficientNet(3, qus_embedding_dim=300, ans_size=kwargs["ans_size"], glove_path=kwargs["glove_path"],
                        word_size=kwargs["word_size"], select_mix_flag=kwargs["select_mix_flag"],
                        mix_probability=kwargs["mix_probability"], mix_alpha_1=kwargs["mix_alpha_1"],
                        mix_alpha_2=kwargs["mix_alpha_2"])
...
if __name__ == '__main__':
    ...
    model = EfficientNet(3).cuda()
    r_m, r_l = model(a, b, c)
    print(r_m.shape, r_l.shape)
    print(sum(m.numel() for m in model.parameters()))  # 5288548  7794184
