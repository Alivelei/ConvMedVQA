...
class ResNetMedVQAModel(nn.Module):
    def __init__(self, block=Bottleneck, num_blocks=[3, 4, 6, 3], qus_embedding_dim=300,
    ...
    def forward(self, x, y):
        ...
        out = self.img_attn(out, y)

        # out = y.mean(1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, out.size(2), out.size(3)) + out
...
def general_resnet34_base(**kwargs):
...
def general_resnet50_base(**kwargs):
    model = ResNetMedVQAModel(
        block=Bottleneck, num_blocks=[3, 4, 6, 3], qus_embedding_dim=300,
        glove_path=kwargs["glove_path"], word_size=kwargs["word_size"],
        ans_size=kwargs["ans_size"],
    )
    return model


def general_resnet101_base(**kwargs):  # 43279627
    model = ResNetMedVQAModel(
...
def general_resnet152_base(**kwargs):
    model = ResNetMedVQAModel(
